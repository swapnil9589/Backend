...
export const registerUser = asyncHandler(async (req, res) => {
  ...
  if ([username, fullname, password, email, mobile_number].some((e) => e?.trim() === ""))
  ...
  if (password.length < 8 || password.length > 20)throw new Apierror(406,"Please fill a strong password (between 8 and 20 characters).");

  //checking mobile_number length
  if (!isValid_mobile_number(mobile_number))throw new Apierror(406, "mobile number is not correct");
  ...
  if (existing_user) throw new Apierror(400, "user already exist please Login");

  //storing pmregistered user in database
  const created_user = await User.create({
    username,fullname: fullname || "",email,mobile_number,password,});

  const data = [created_user];
  res.status(200).cookie().send(new Apiresponse(201, data, "user created successfully"));

  //storing profile if exist
  let profileimage;
  let image_url;
  if (Array.isArray(await req?.files?.Profile_Image) &&req.files.Profile_Image[0].path &&req.files) {
  ...
  let coverimage;
  let coverimage_url;
  if (Array.isArray(await req?.files?.coverImage) &&req.files.coverImage[0].path &&req.files) {
...
//find user and saving refresh token in user db
  const user = await User.findByIdAndUpdate(
    created_user._id,
    {
      $set: {profile_photo: image_url,cover_photo: coverimage_url,refresh_token: refreshtoken}
    },{ new: true });
